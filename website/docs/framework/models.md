...
A **Model** is how Sublime UI thinks about your data. Inspired by Laravel's
Eloquent, you declare a class **once** — its fields and its resource — and that
single declaration becomes the home for reading, writing, caching, and reacting
to that data on mobile, web, and desktop.

The same model code runs everywhere. What changes is **where the data lives** —
and you choose that per model, without touching the model itself.

:::note
Sublime is pre-1.0. This page documents the **0.2 model layer**, where a model's
backend is a pluggable *gateway* (in-memory by default, with local-DB and REST
options). Earlier 0.1.x releases defaulted to REST.
:::
...
- **Desktop** → SQLite (`better-sqlite3`), reached through the desktop native
  bridge; the web build automatically upgrades to it when running in the desktop
  shell.
...
- [Framework overview](/docs/framework/overview)
